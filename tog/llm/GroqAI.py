...
class GroqAILLM(BaseLLM):
    ...
    def extract_initial_entities(self, query: str) -> List[str]:
        """
        Extract initial entities from the input query.
        ...
        """
        logger.info(f"Extracting initial entities for query: {query}")
        prompt = str(Prompts.EXTRACT_ENTITIES).format(query=query)
    ...
    def evaluate_paths(self, paths: List[Dict[str, Any]]) -> bool:
        """
        ...
        prompt = str(Prompts.EVALUATE_PATHS).format(paths_str=paths_str)
        response = self._get_completion(prompt).lower()
        return response.startswith('yes')

    def generate_answer(self, paths: List[Dict[str, Any]]) -> str:
        """
        ...
        prompt = str(Prompts.GENERATE_ANSWER).format(paths_str=paths_str)
