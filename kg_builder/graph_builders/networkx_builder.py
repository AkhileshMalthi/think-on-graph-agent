...
class NxKG:
    ...
    def load_from_pipeline_json(self, json_data):
        ...
        for entity in json_data.get("entities", []):
            ...
            self.add_entity(name, id=entity_id, type=entity_type, **metadata)
    ...
    def _create_clean_tooltip(self, data, tooltip_type="entity"):
        """
        Create a clean, well-formatted plain text tooltip for PyVis compatibility.
        
        Args:
            data (dict): Entity or relationship data
            tooltip_type (str): Type of tooltip ('entity' or 'relationship')
        
        Returns:
            str: Clean plain text tooltip
        """
        tooltip_lines = []
        
        # Add header with entity/relationship name
        if tooltip_type == "entity":
            name = data.get("name", "Unknown")
            tooltip_lines.append(f"📍 {name}")
            tooltip_lines.append("=" * (len(name) + 3))
        else:
            source = data.get("source", "")
            target = data.get("target", "")
            tooltip_lines.append(f"🔗 {source} → {target}")
            tooltip_lines.append("=" * 20)

        # Add type
        if "type" in data and data["type"]:
            emoji = "👤" if tooltip_type == "entity" else "🔗"
            tooltip_lines.append(f"{emoji} Type: {data['type']}")

        # Add ID if available
        if "id" in data and data["id"]:
            tooltip_lines.append(f"🆔 ID: {data['id']}")
        ...
        # Add chunk and section info
        chunk_info = []
        if "chunk_id" in data and data["chunk_id"]:
            chunk_info.append(f"Chunk: {data['chunk_id']}")
        if "section_id" in data and data["section_id"]:
            chunk_info.append(f"Section: {data['section_id']}")
        
        if chunk_info:
            tooltip_lines.append(f"📄 {' | '.join(chunk_info)}")

        # Add strength for relationships
        if tooltip_type == "relationship" and "strength" in data:
            strength_emoji = {"strong": "💪", "medium": "👍", "weak": "👌"}.get(str(data["strength"]).lower(), "🔗")
            tooltip_lines.append(f"{strength_emoji} Strength: {data['strength']}")

        # Add attributes if available
        if "attributes" in data and isinstance(data["attributes"], dict) and data["attributes"]:
            tooltip_lines.append("🏷️ Attributes:")
            for attr_key, attr_value in data["attributes"].items():
                if attr_value is not None:
                    tooltip_lines.append(f"  • {attr_key}: {attr_value}")
    ...
    def visualize(
        ...
        """
        Visualize the knowledge graph using PyVis with improved tooltips.
        ...
        for node, data in self.graph.nodes(data=True):
            ...
            tooltip_data = {"name": node, **data}
            tooltip = self._create_clean_tooltip(tooltip_data, "entity")

            # Set node size based on number of connections
            node_degree = self.graph.degree(node)
            size = max(15, min(40, 15 + node_degree * 2))
        ...
        # Add edges with clean tooltips
        for source, target, data in self.graph.edges(data=True):
            # Create clean tooltip for edges
            tooltip_data = {"source": source, "target": target, **data}
            tooltip = self._create_clean_tooltip(tooltip_data, "relationship")
            ...
            if "strength" in data:
                ...
                edge_color = strength_color_map.get(str(data["strength"]).lower(), "#808080")
            ...
            if "strength" in data:
                ...
                edge_width = strength_width_map.get(str(data["strength"]).lower(), 2)
    ...
    def get_entities_by_property(self, property_name, property_value):
        ...
        """
        return [node for node, data in self.graph.nodes(data=True) if data.get(property_name) == property_value]
    ...
    def export_to_json(self):
        ...
        """
        entities = [{"name": node, **data} for node, data in self.graph.nodes(data=True)]

        relationships = [{"source": u, "target": v, **data} for (u, v, data) in self.graph.edges(data=True)]
...
def main():
    """
    Example usage of the updated NxKG class with improved tooltips
    """
    # Create a knowledge graph instance
    kg = NxKG()

    # Load from your pipeline JSON file
    kg.load_from_json_file(r"complete_knowledge_graph.json")
    ...
    # Create visualization with improved tooltips
    kg.visualize(
