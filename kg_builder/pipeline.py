...
from kg_builder.extractors.advanced import (
    KnowledgeGraphExtractor,
    Entity,
    Relationship,
)
from kg_builder.connectors.neo4j import Neo4jConnector
...
class KnowledgeGraphPipeline:
    ...
    def enhance_relationships_with_chunk_info(
        self, relationships: List[Relationship], chunk: ChunkData
    ) -> List[Relationship]:
        """
    ...
    def process_single_chunk(self, chunk: ChunkData) -> Dict[str, List]:
        ...
        try:
            ...
            enhanced_entities = self.enhance_entities_with_chunk_info(deduped_graph["entities"], chunk)
            enhanced_relationships = self.enhance_relationships_with_chunk_info(deduped_graph["relationships"], chunk)
    ...
    def process_all_chunks(self) -> Dict[str, List]:
        ...
        logger.info(
            f"Final graph: {len(final_graph['entities'])} entities, {len(final_graph['relationships'])} relationships"
        )
    ...
    def save_to_file(
        ...
        filename: str = "complete_knowledge_graph.json",
    ):
        """
    ...
    def run_pipeline(
        ...
        output_filename: str = "complete_knowledge_graph.json",
    ):
        """
...
def main():
    ...
    """
    # Configuration
    CHUNKS_FOLDER = r"markdown_chunks2\legal_dataset_1_processed\chunks"  # Modify this path as needed
    OUTPUT_FILE = "legal3.json"

    try:
        # Initialize pipeline
        pipeline = KnowledgeGraphPipeline(chunks_folder=CHUNKS_FOLDER, similarity_threshold=0.8)
